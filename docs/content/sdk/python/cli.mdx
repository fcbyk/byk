...
`wait_for_server_ready` 和 `open_browser` 为标准库实现，无需额外依赖。

:::warning 剪贴板
`copy_to_clipboard` 需要 `pyperclip`：
...
缺失时 byksdk 会抛出清晰的 `ImportError` 并提示安装命令。
:::
...
```py
import click
from byksdk import colored_key_value
...
<Tab label="使用示例">
```py
from byksdk import echo_network_urls, get_private_networks
...
<Tab label="使用示例">
```py
from byksdk import copy_to_clipboard
...
import threading
import webbrowser
from byksdk import wait_for_server_ready
...
<Tab label="使用示例">
```py
from byksdk import open_browser
