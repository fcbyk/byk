...
```
1. $pip                  → 全局共享 pip 依赖（不属于任何插件，卸载时保留）
...
`$pip` 是根级别的共享 pip 依赖，不属于任何插件，**卸载时保留**。支持单字符串或字符串数组写法：
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `$pip` | string \| string[] | 否 | 全局共享 pip 依赖，安装任何插件时都会安装，卸载时保留 |

> `$pip` 适合声明多个插件共享的基础依赖，避免每个插件重复声明。与插件的 `pip` 字段不同，`$pip` 中的包不会随任何插件的卸载而删除。
...
- 操作块按固定顺序执行：`$pip` → `pip` → `downloads` → `download-to-workdir` → `download-to-alias` → `commands` / `command` → `alias`
- 使用 `--cdn` 时，所有 `raw.githubusercontent.com` 的 URL 自动转换为 jsDelivr CDN 加速
- 顶层 `$var`/`$default`/`$pip` 为系统字段，不会被视为有效 plugin key
